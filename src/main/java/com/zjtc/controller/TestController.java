...
public class TestController {
    ...
    public ResponseEntity<byte[]> test(@RequestHeader("Device-ID") String deviceId, @RequestBody ConsumTransactionsDto consumTransactionsDto) {
        ...
        if (consumTransactionsDto.getMode() == 1) {
            ...
            boolean isMax = watConsumeEmployeeCountService.checkDailyMaxConsumeTimes(viewEmployeeData.getEmployeeID(), watDeviceparameter);
            if (isMax)
                return ResponseEntity.ok(responseHelper.constructionResult(0, "超过当天日消费次数", employeeBags, grantsEmployeeBags, consumTransactionsDto, BigDecimal.ZERO, deviceId, byId, watDeviceparameter, false));
            ...
            log.info("返回的余额{}", vo1);
            return ResponseEntity.ok(responseHelper.balance(vo1));
        }
        //扣费
        if (consumTransactionsDto.getMode() == 0) {
            ...
            if (devicePayModeID == 0) {
                ...
                if (deviceConModeID == 1) {
                    ...
                    if (new BigDecimal(requestAmount).compareTo(new BigDecimal(preAmountForTime)) >= 0) {
                        return priorityTypeDecision(watDevice, employeeBags, new BigDecimal(vo.getAmount()).divide(new BigDecimal(10)), grantsEmployeeBags, consumTransactionsDto, cardData, deviceId, byId, watDeviceparameter, true);
                    } else {
                        return priorityTypeDecision(watDevice, employeeBags, new BigDecimal(vo.getAmount()), grantsEmployeeBags, consumTransactionsDto, cardData, deviceId, byId, watDeviceparameter, true);
                    }
                } else {
